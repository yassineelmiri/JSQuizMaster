...
module.exports = {
  ...
  getAllProfessors: (req, res) => {
    professor
      ...
      .catch((error) => {
        ...
        res
          .status(500)
          .json({ message: "An error occurred while fetching professors." });
      });
  },
  getProfessorsResulte: (req, res) => {
    professor
      .getProfessorsResulte()
      .then((result) => {
        res.status(200).send(result);
      })
      .catch((error) => {
  ...
  getOneProfessor: (req, res) => {
    ...
    professor
      ...
      .catch((error) => {
        ...
        res.status(500).json({
          message: `An error occurred while fetching professor with this id : ${id}`,
        });
      });
  },
  ...
  assignTestToStudents: async (req, res) => {
    ...
    try {
      ...
      res.redirect("/assign-quiz");
    } catch (error) {
      console.error("Error assigning quiz:", error);
      res
        .status(500)
        .json({ message: "An error occurred while assigning the quiz." });
    }
  },
  ...
  renderAssignTestPage: async (req, res) => {
    try {
      const students = await student.getAllStudents(); // Retrieve students
      const tests = await test.getAllTests(); // Retrieve tests

      // Pass both students and tests to the EJS template
      res.render("Formateur/Assign", { students, tests });
    } catch (error) {
